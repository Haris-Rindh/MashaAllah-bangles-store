...
import { getProductById, getAllProducts } from '../store/productStore'
...
export default function ProductDetail() {
  ...
  // Support both 'desc'+'longDesc' (static) and 'description' (admin-added)
  const shortDesc = product.desc        || product.description || ''
  const longDesc  = product.longDesc    || product.description || shortDesc
  const price     = product.discountPrice || product.price || product.regularPrice || 0
  const origPrice = product.regularPrice  || product.price || 0

  const handleAdd = () => {
    addToCart({ ...product, price })   // cart always uses effective price
    setAdded(true)
  ...
  const related = getAllProducts()
    .filter(p => p.category === product.category && (p.id || p._id) !== id)
    .slice(0, 4)
    ...
    <div style={{ background: '#FDFBF7', minHeight: '100vh' }}>
      ...
      <div className="pt-24 pb-4 px-6 md:px-16 lg:px-24" style={{ borderBottom: '1px solid #E4D5C8' }}>
        <div className="max-w-6xl mx-auto flex items-center gap-2 text-xs" style={{ color: '#6B5548' }}>
          <Link to="/" style={{ color: '#C9906A' }}>Home</Link>
          <span>/</span>
          <Link to="/shop"
            onMouseEnter={e => e.target.style.color='#C9906A'}
            onMouseLeave={e => e.target.style.color='#6B5548'}>Shop</Link>
          <span>/</span>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          ...
          <div className="flex flex-col">
            <div className="flex items-center gap-3 mb-4">
              ...
              {product.tag && (
                <span className="text-[10px] font-medium tracking-widest uppercase px-3 py-1.5 rounded-full"
                  style={{ background: `${product.color || '#C9906A'}18`, color: product.color || '#C9906A', border: `1px solid ${product.color || '#C9906A'}35` }}>
                  {product.tag}
                </span>
              )}
            </div>
            ...
            <div className="flex items-baseline gap-3 mb-6 pb-6" style={{ borderBottom: '1px solid #E4D5C8' }}>
              ...
              {product.discountPrice && origPrice > product.discountPrice && (
                <span className="text-sm line-through font-light" style={{ color: '#C4AEAB' }}>
            ...
            <div className="flex items-end justify-between mb-8">
              <div>
                ...
                <h2 className="font-display text-2xl md:text-3xl font-semibold text-charcoal">
                  More <em className="not-italic gradient-text">{catLabel[product.category] || product.category}</em>
                </h2>
              </div>
              <Link to={`/shop?cat=${product.category}`} className="btn-ghost hidden sm:block"
                style={{ padding: '0.55rem 1.3rem', fontSize: '0.72rem' }}>See All</Link>
            </div>
