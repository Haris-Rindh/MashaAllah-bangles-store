...
export default function ProductCard({ product }) {
  ...
  return (
    <div className="prod-card group relative flex flex-col h-full bg-white text-swa-dark">
      <Link to={`/product/${product.id}`} className="block relative overflow-hidden prod-img-wrap mb-4">
      ...
      <button onClick={() => addToCart(product.id, 1)} className="prod-bag border-none cursor-pointer">Add to Bag</button>
    </div>
  )
}
